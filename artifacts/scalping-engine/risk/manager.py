...
Checks:
  ...
  • Minimum risk:reward ratio met (strict >= 2.0, no tolerance)
  ...
  • Max risk % per trade not exceeded (3% of account balance)
...
def validate(decision, session_stats: dict) -> tuple[bool, str]:
    ...
    All limits are read live from config so dashboard settings take effect
    ...
    # ── Confidence gate (safety net — strategies also check this) ─────────────
    ...
    # ── Minimum SL size gate (safety net — strategies also check this) ────────
    ...
    if risk_amount > max_risk_amount:
        return False, (
            ...
            f"account too small for this structure"
        )
